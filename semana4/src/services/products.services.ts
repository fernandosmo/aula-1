import { iNewProduct, iProduct, iProductEdit } from "../models/products.model";
...
class ProductsService {
  ...
  create(product: iNewProduct) {
    const id: number = this.products[this.products.length - 1].id + 1;
    const newProduct: iProduct = { id, ...product }
    
    this.products.push(newProduct)
  }

  update(id: number, productUpdated: iProductEdit) {
    const index = this.products.findIndex(product => product.id === id);
